...
std::string outputFolder = "ProvaPGM";
	...
	for (uint8_t j = minQuality; j <= maxQuality; j++)
	{
		...
		DSTR_PROFILE_SCOPE("Quality");
		{
			...
			case ImageFormat::WEBP:
					ImageWriter::WriteWEBP(currPath + ss.str() + ".webp", encoded, width, height, 0);
					break;
#endif
			}
		}
	...
	for (uint32_t c = 0; c < 7; c++)
	{
		...
		for (uint32_t q = 0; q < 4; q++)
		{
			...
			for (uint32_t p = 0; p < algoBits.size(); p++)
			{
				...
				if (!coders[c].compare("Hilbert")) BenchmarkCoder<Hilbert>(quantizations[q], true, algoBits[p], quantizedData, encodedData, decodedData, colorBuffer, originalData, dmData.Width, dmData.Height, folders, "Hilbert", ImageFormat::PNG);
				if (!coders[c].compare("Morton")) BenchmarkCoder<Morton>(quantizations[q], true, algoBits[p], quantizedData, encodedData, decodedData, colorBuffer, originalData, dmData.Width, dmData.Height, folders, "Morton", ImageFormat::PNG);
				if (!coders[c].compare("Hue")) BenchmarkCoder<Hue>(quantizations[q], true, algoBits[p], quantizedData, encodedData, decodedData, colorBuffer, originalData, dmData.Width, dmData.Height, folders, "Hue", ImageFormat::PNG);
				if (!coders[c].compare("Triangle")) BenchmarkCoder<Triangle>(quantizations[q], true, algoBits[p], quantizedData, encodedData, decodedData, colorBuffer, originalData, dmData.Width, dmData.Height, folders, "Triangle", ImageFormat::PNG);
				if (!coders[c].compare("Split")) BenchmarkCoder<Split>(quantizations[q], true, algoBits[p], quantizedData, encodedData, decodedData, colorBuffer, originalData, dmData.Width, dmData.Height, folders, "Split", ImageFormat::PNG);
				if (!coders[c].compare("Phase")) BenchmarkCoder<Phase>(quantizations[q], true, algoBits[p], quantizedData, encodedData, decodedData, colorBuffer, originalData, dmData.Width, dmData.Height, folders, "Phase", ImageFormat::PNG);
				if (!coders[c].compare("Packed")) BenchmarkCoder<Packed>(quantizations[q], false, algoBits[p], quantizedData, encodedData, decodedData, colorBuffer, originalData, dmData.Width, dmData.Height, folders, "Packed", ImageFormat::PNG);
